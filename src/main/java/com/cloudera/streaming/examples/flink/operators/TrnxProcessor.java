...
/**
 * Core transaction and query processing logic. {@link #processElement1(ITrnx2, Context, Collector)} receives
 ...
 */
public class TrnxProcessor extends KeyedCoProcessFunction<String, ITrnx2, Query, TrnxResult> {
    ...
    @Override
    public void processElement1(ITrnx2 transaction, Context ctx, Collector<TrnxResult> out) throws Exception {
        long startTime = System.nanoTime();
        ItemInfo info = itemState.value();
        itemRead.update(System.nanoTime() - startTime);

        if (info == null) {
            info = new ItemInfo(transaction.getItemId().toString(), 0);
        }
        int newQuantity = info.quantity + transaction.getQty();
        ...
        out.collect(new TrnxResult(transaction, success));
    }

    @Override
    public void processElement2(Query query, Context ctx, Collector<TrnxResult> out) throws Exception {
