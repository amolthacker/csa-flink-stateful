...
import com.cloudera.streaming.examples.flink.operators.*;
 ...
 * subclasses have to implement input and output methods. Check the {@link KafkaHDFSITrnxJob} for a Kafka input/output based
 * implementation of the pipeline.
 */
public abstract class ITrnxJob {
    ...
    public final StreamExecutionEnvironment createApplicationPipeline(ParameterTool params) throws Exception {
        ...
        DataStream<ITrnx2> transactionStream = readTransactionStream(params, env);
        ...
        SingleOutputStreamOperator<TrnxResult> processedTransactions = transactionStream.keyBy("itemId")
                .connect(queryStream.keyBy("itemId"))
                .process(new TrnxProcessor())
        ...
        if (params.getBoolean(ENABLE_SUMMARIES_KEY, false)) {
            DataStream<TransactionSummary> transactionSummaryStream = processedTransactions
                    ...
                    .aggregate(new TrnxSummaryAggregator())
    ...
    protected abstract DataStream<ITrnx2> readTransactionStream(ParameterTool params, StreamExecutionEnvironment env);

    protected abstract void writeQueryOutput(ParameterTool params, DataStream<QueryResult> queryResultStream);

    protected abstract void writeTransactionResults(ParameterTool params, DataStream<TrnxResult> transactionResults);
