...
import com.cloudera.streaming.examples.flink.types.TrnxResult;
import com.cloudera.streaming.examples.flink.types.TransactionSummary;

public class TrnxSummaryAggregator implements AggregateFunction<TrnxResult, TransactionSummary, TransactionSummary> {

    @Override
    public TransactionSummary createAccumulator() {return new TransactionSummary();}

    @Override
    public TransactionSummary add(TrnxResult tr, TransactionSummary acc) {
        acc.itemId = tr.transaction.getItemId().toString();
        if (tr.success) {
            acc.numSuccessfulTransactions++;
        } else {
            acc.numFailedTransactions++;
        }
        acc.totalVolume += Math.abs(tr.transaction.getQty());
        return acc;
    }
